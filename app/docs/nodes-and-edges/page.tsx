...
export default function NodesAndEdgesPage() {
      ...
      <Prose>
        ...
        <Table
          ...
          rows={[
            ...
            [<InlineCode key="i">isIntercepted, isParallelSlot</InlineCode>, "Advanced App Router features"],
            [<InlineCode key="rt">runtime, rendering</InlineCode>, "nodejs/edge and static/dynamic/isr"],
        ...
        <Table
          ...
          rows={[
            ...
            [<InlineCode key="rd">redirect / permanent-redirect</InlineCode>, "Server or client redirect"],
            [<InlineCode key="rw">rewrite</InlineCode>, "URL rewrite (middleware/config)"],
            [<InlineCode key="lp">layout-parent / template-parent</InlineCode>, "Composition relationships"],
        ...
        <P>A trimmed example of <InlineCode>graph.json</InlineCode>:</P>
        <CodeBlock
          language="json"
  ...
  "metadata": { "totalRoutes": 1, "totalLayouts": 1, "deadRouteCount": 0 }
        ...
        <P>
          ...
          <InlineCode>SerializedGraph</InlineCode> type from <InlineCode>@route-intelligence/shared</InlineCode>{" "}
          or use the <A href="/docs/api">programmatic API</A> to work with a live{" "}
...
const deadRoutes = (graph as SerializedGraph).nodes.filter(
  (node) => node.attributes.isDead,
        ...
        />
        <P>
          <Strong>Tip:</Strong> the same fields drive the <A href="/docs/diagnostics">diagnostics</A>{" "}
