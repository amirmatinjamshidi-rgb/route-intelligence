...
export default function DiagnosticsPage() {
    ...
    >
      <Prose>
        ...
        <Table
          ...
          rows={[
            [<InlineCode key="dr">dead-route</InlineCode>, "warning", "A route no link or redirect can reach"],
            [<InlineCode key="dl">dead-layout</InlineCode>, "warning", "A layout wrapping no reachable route"],
            [<InlineCode key="bl">broken-link</InlineCode>, "error", "A link to a path that doesn't exist"],
            [<InlineCode key="br">broken-redirect</InlineCode>, "error", "A redirect to a missing target"],
            [<InlineCode key="rc">redirect-cycle</InlineCode>, "error", "Redirects that loop forever"],
            [<InlineCode key="cn">circular-navigation</InlineCode>, "warning", "A navigation cycle between routes"],
            [<InlineCode key="sr">shadowed-route / duplicate-route</InlineCode>, "warning", "Two routes resolving to one path"],
            [<InlineCode key="or">open-redirect</InlineCode>, "error", "A redirect target derived from user input"],
            [<InlineCode key="me">missing-error-boundary / missing-loading</InlineCode>, "info", "Recommended files are absent"],
          ]}
        />
  ...
  edgeId?: string;
        ...
        <Callout kind="tip" title="Same data everywhere">
          ...
          <A href="/docs/integrations">ESLint plugin</A>, the VS Code Problems panel, and PR comments
          from the GitHub Action. <Strong>Write a rule once, surface it everywhere.</Strong>
        </Callout>
      </Prose>
    </DocPage>
  );
